                  ...
                  <div className="place-items-center text-[black] w-full">
                    <div className=" px-6 py-3 rounded-2xl bg-white w-full">
                      ...
                      <div className="m-1">
                        ...
                        {confirmed_at ? `Yes ${confirmed_at}` : 'Account not confirmed' }
                      </div>
                      <div className="m-1">
                        <label className="block text-l font-bold mb-2">
                          ...
                          <thead className="bg-primary">
                            <tr className="border-b border-primary">
                              ...
                              <th className="text-center p-4 text-s font-normal text-white">
                                { activeTab === 'receipts' ? 'Transaction Type' : 'Total Cost'}
                              </th>
                              <th className="text-center p-4 text-s font-normal text-white">
                                { activeTab === 'receipts' ? 'Amount' : 'Cart Status'}
                              </th>
                              <th className="text-center p-4 text-s font-normal text-white">
                                { activeTab === 'receipts' ? 'Balance Before & After' : 'Paid?'}
                              </th>
                              <th className={`p-4 text-s font-extralight text-white text-center`}>
                                { activeTab === 'receipts' ? 'Description' : 'Social program'}
                              </th>
                            ...
                            : activeTab === 'orders' && filteredOrders.length > 0 ? (
                              filteredOrders.map((order: UserCartOrder, index: number) => (
                                ...
                                >
                                  ...
                                  <td className={`p-4 text-m text-center ${order.cart_status === 'approved' ? 'text-green-700' : order.cart_status === 'rejected' ? 'text-red-800' : 'text-orange-500' }`}>
                                    {order.cart_status}
                                  </td>
                                  <td className={`p-4 text-m text-center ${ order.is_paid ? 'text-green-700' : 'text-red-800' }`}>
                                    {order.is_paid ? 'Yes' : 'No'}
                                  </td>
                                  <td className={`p-4 text-m  ${activeTab === 'orders' ? 'text-center' : 'text-right'}`}>
                                    {order.social_program.title}
                                  </td>
                                  <td className={`p-4 text-m  ${activeTab === 'orders' ? 'text-center' : 'text-right'} hover:text-primary hover:underline`}>
                                    <NavLink to={`/user_cart_order/${order.id}`}>View User Cart Order Info</NavLink>
                                  </td>
                                </tr>
                              ))
                              ...
                              filteredReceipts.map((receipt: Receipt, index: number) => (
                                ...
                                >
                                  ...
                                  <td className="p-4 text-m text-center">
                                    {receipt.balance_before} - {receipt.balance_after}
                                  </td>
                                  <td className={`p-4 text-m  ${activeTab === 'receipts' ? 'text-center' : 'text-right'}`}>
                                    {receipt.description}
                                  </td>
                                  <td className="p-4">
                                    <div className="flex items-center text-m justify-center gap-2 hover:text-primary hover:underline">
                                      <NavLink to={`/receipts/${receipt.id}`}>View Receipt Info</NavLink>
                                    </div>
                                  </td>
                                </tr>
                              ))
